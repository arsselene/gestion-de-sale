...
export function ClassroomsPage() {
  const { classrooms, classSessions, addClassroom, deleteClassroom } = useAppStore()
  ...
  const getClassForSlot = (classroomId: string, day: string, timeSlot: string) => {
    const [startTime] = timeSlot.split('-')
    return classSessions.find(
      (session) =>
        session.location === classroomId && session.day === day && session.startTime === startTime
    ) || null
  }

  const getClassesInRoom = (classroomId: string) => classSessions.filter((c) => c.location === classroomId)

  return (
    <div className="p-8">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-foreground">Classrooms Weekly Schedule</h2>
        <p className="text-muted-foreground mt-2">Manage classrooms and view schedules</p>
      </div>
          ...
          <Card className="p-4 h-fit">
            ...
            <div className="space-y-2">
              {classrooms.map((classroom) => (
                <div key={classroom.id} className="flex items-center gap-2 group">
                  <button
                  ...
                  >
                    ...
                    <div className="text-xs opacity-75">{getClassesInRoom(classroom.id).length} classes</div>
                  </button>
                  <button
                          ...
                          <td key={`${day}-${timeSlot}`} className="py-3 px-3">
                            {classSession ? (
                              <div className="bg-blue-500/10 border border-blue-200 rounded px-2 py-2 text-xs">
                                ...
                                <div className="text-blue-700 text-[10px]">{classSession.type}</div>
                                <div className="text-blue-600 text-[10px]">{classSession.instructor}</div>
                              </div>
                            ) : (
              ...
              {classrooms.map((classroom) => (
                <div key={classroom.id} className="border border-border rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="px-3 py-1 rounded text-xs font-medium bg-green-500/10 text-green-700">
                      Available
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground">{getClassesInRoom(classroom.id).length} classes scheduled</p>
                </div>
              ))}
            </div>
          </Card>
      ...
      {showAddDialog && (
        <AddClassroomDialog onClose={() => setShowAddDialog(false)} />
      )}
    </div>
  )
}
